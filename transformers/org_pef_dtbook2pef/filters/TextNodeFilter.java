...
public class TextNodeFilter extends StaxFilter {
	 ...
	 */
	public TextNodeFilter(XMLEventReader input, OutputStream output, ArrayList<StringFilter> filters) throws XMLStreamException, FileNotFoundException {
		super(input, output);
		this.filters = filters;
	}
	
    protected Characters characters(Characters event) {
    	String ret = event.getData();
    	for (StringFilter e : filters) {
    		ret = e.replace(ret); 
    	}
    	return getEventFactory().createCharacters(ret);
    }
