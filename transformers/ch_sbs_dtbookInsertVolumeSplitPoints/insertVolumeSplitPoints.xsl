...
<!-- This stylesheet inserts split points in a dtbook so that a book
     ...
     If endnotes=document, notes are expected to have already been
     moved to the end of the document. In that case notes are left out
     of the word count entirely: the target size is still based on the
     full word count (including notes), but only real content is
     walked when looking for split points, so the walk runs out of
  ...
  <!-- Generally the volumes do not seem to be filled to capacity with
       ...
       percentage. This does not apply when endnotes=document: there
       notes are already left out of the word count (see above), and
       inflating the target on top of that can make the requested
       number of volumes unreachable. -->
  ...
  <xsl:function name="func:splitInternal">
    ...
    <xsl:choose>
      ...
      <!-- Never split inside a note/annotation: their content isn't
           rendered at this position, so a split point here wouldn't correspond
           to any real content boundary. -->
      <xsl:when test="$wordsSoFar >= $wordsPerVolume and not($head/ancestor::dtb:note or $head/ancestor::dtb:annotation)">
  ...
  <!-- Do not consider notes/annotations when looking for split points in case of $endnotes = 'document'. -->
  <xsl:variable name="core_p"
    select="if ($endnotes = 'document')
            then $all_p[not(ancestor::dtb:note or ancestor::dtb:annotation)]
            else $all_p"/>
  <xsl:variable name="words_per_volume"
    select="ceiling($total_words div number($number_of_volumes))
            * (1 + (if ($endnotes = 'document') then 0 else $words_per_volume_increase))"/>
  <xsl:variable name="split_nodes" select="func:split($words_per_volume, $core_p)"/>
