...
public class Xhtml2DTBook extends Transformer implements FilesetErrorHandler {
	...
	protected boolean execute(Map parameters) throws TransformerRunException {
		try {
			...
			// get input file
			File input = FilenameOrFileURI.toFile((String) parameters
					.remove("input"));

			// prep output dir
			File output = FilenameOrFileURI.toFile((String) parameters
					.remove("output"));
			...
			Stylesheet.apply(input.getAbsolutePath(), xsltURL, output
					...
					parameters, CatalogEntityResolver.getInstance());
