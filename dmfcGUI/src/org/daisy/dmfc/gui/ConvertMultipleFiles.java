...
//		Composite Output stuff
	...
	public void setDirectorySelected() {
		...
		for (int i = 0; i<size; i++){
			...
			if (dirSelected!=null && dirSelected.equalsIgnoreCase(rootPath)){
				...
				//can't choose c:\, too long to parse
	...
	/**
	 * Methods used to populate compatible files table
	 *
	 */
		...
		try {
			...
			if (type != null) {
				...
				for (int i = 0; i < arr.length; ++i) {
					...
					System.out.println("getGlob From Mime, what is it?" + ret[i]);
...
//			compare them to the mimetype pattern(s) and 
			...
			System.out.println("The length of the pattern array is " + mimePatternExtensions.length);

			for (int k=0; k<mimePatternExtensions.length; k++){
				String type = mimePatternExtensions[k];
				
				System.out.println("What is Strend?"  + strEnd);
...
//					compare them to the mimetype pattern(s) and 
	...
	public String addFileNameToOutputPath(String outPath, File inPath){
		...
		if (intOutputIsDir==2){
			...
			String strInPath = inPutPath.getName();
	...
	public void setFileOrDirFlag(){
		...
		String [] arFiles = this.getGlobFromMime(this.getMimeForProperty("output"));
		String [] arMatches = null;
		...
		if (arFiles!=null){
			...
			for (int i = 0; i<arFiles.length; i++){
				String mimePattern= getGlobExtension(arFiles[i]);
				arMatches[i]=mimePattern;
				System.out.println("The output mimePattern is " + mimePattern);
			}	
		}
		...
		if (tblCompatibleFiles.getItemCount()==0 
			...
			if (makeDirFile.exists() && makeDirFile.isFile()){
				...
				for(int i = 0; i<count;i++){
					...
					//if(boolOutputIsDir==false){
					if(intOutputIsDir==2){
						//if the output is a file then add the file name
