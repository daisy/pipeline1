...
public class XhtmlTitleProvider {
	...
	/**
	 * Retrieve a title descriptor of inparam XHTML document; html/head/title
	 * has precedence, followed by the first occuring html/body/hx, if any.
	 * 
	 * @param xhtmlDoc
	 ...
	 */
	public static String getTitle(URL xhtmlDoc) throws IOException,
			XMLStreamException {
		...
		try {
			...
			xif.setXMLResolver(new StaxEntityResolver(CatalogEntityResolver
					.getInstance()));
			XMLStreamReader xsr = xif.createXMLStreamReader(xhtmlDoc
					.openStream());
			while (xsr.hasNext()) {
				xsr.next();
