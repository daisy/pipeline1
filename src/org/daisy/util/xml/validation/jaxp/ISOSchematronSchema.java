 ...
 */
public class ISOSchematronSchema extends AbstractSchema implements ErrorListener {
	private TransformerFactory transformerFactory = null;
	private Transformer mRngTransformer = null;
	private Transformer mAnyTransformer = null;
	
	ISOSchematronSchema(URL schema, SchemaFactory originator) throws 	IOException, TransformerConfigurationException, 
	...
	ISOSchematronSchema(Source[] sources, SchemaFactory originator) throws IOException, TransformerConfigurationException, 
																		XSLTException, PoolException, SAXException {
	...
	public Validator newValidator() {
		ISOSchematronValidator validator = new ISOSchematronValidator(this);
	...
	private Source[] transform(Source[] inSources) throws TransformerConfigurationException, IOException, 
		...
		if(null==transformerFactory) {
			...
			mRngTransformer = transformerFactory.newTransformer
				(new StreamSource(Stylesheets.get("RNG2ISOSchtrn.xsl").openStream()));
			mAnyTransformer = transformerFactory.newTransformer
				(new StreamSource(Stylesheets.get("Any2ISOSchtrn.xsl").openStream()));
		}
		...
		for (int i = 0; i < inSources.length; i++) {
			...
			} else if (peekResult.getRootElementNsUri().equals(SchemaLanguageConstants.ISO_SCHEMATRON_NS_URI)) {
